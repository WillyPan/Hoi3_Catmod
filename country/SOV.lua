	...
	if lbAtWarGER or (liCYear < 1942 and not(voTechnologyData.IsAtWar)) then
		...
		if lbControlMoscow and liCYear > 1942 then
			...
			laTechWeights = {
				...
				0.24, -- landDoctrinesWeight
				0.18, -- airBasedWeight
				0.16, -- airDoctrinesWeight
...
-- Create Custom infantry for SOV
function P.Build_infantry_brigade(vIC, viManpowerTotal, voType, voProductionData, viUnitQuantity)
	-- Build old style formations till 1943
	if voProductionData.Year < 1943 and not(voProductionData.IsExile) then
...
-- Create Custom heavy_tank for SOV
function P.Build_heavy_armor_brigade(vIC, viManpowerTotal, voType, voProductionData, viUnitQuantity)
	-- Build new style formations after 1942
	if voProductionData.Year > 1942 and not(voProductionData.IsExile) then
...
-- We cut our mass building of industry off so now look at air bases
   if voProductionData.Year > 1938 then
		ic = Support.Build_AirBase(ic, voProductionData, 1694, 8) -- Minsk
		ic = Support.Build_AirBase(ic, voProductionData, 2223, 8) -- Kyiv
		ic = Support.Build_AirBase(ic, voProductionData, 1991, 8) -- Homel
...
function P.Build_Radar(ic, voProductionData)
	if voProductionData.Year > 1938 then
		-- Ok to build a few
		ic = Support.Build_Radar(ic, voProductionData, 1409, 3) -- Moskva
		ic = Support.Build_Radar(ic, voProductionData, 782, 3) -- Leningrad
		ic = Support.Build_Radar(ic, voProductionData, 1991, 3) -- Homel
		ic = Support.Build_Radar(ic, voProductionData, 2401, 3) -- Kharkov
