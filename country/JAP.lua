...
function P.ProductionWeights(voProductionData)
	...
	if voProductionData.ManpowerTotal < 150 then
		...
		if not(lbUSAWar) then
			...
			local lbControlBeiping = (CCurrentGameState.GetProvince(4979):GetController() == chiTag)
			-- If China still controls Shanghai keep hitting them
			if voProductionData.Year <= 1938 or lbControlBeiping then
				laArray = {
					0.45, -- Land
					0.15, -- Air
					0.25, -- Sea
...
function P.LandRatio(voProductionData)
	...
	if voProductionData.Year <= 1938 then
		laArray = {
			garrison_brigade = 1,
			infantry_brigade = 10,
			light_armor_brigade = 0.3};
        else
		laArray = {
			garrison_brigade = 2,
			infantry_brigade = 9,
			light_armor_brigade = 0.1};
	end
...
function P.AirRatio(voProductionData)
	...
	laArray = {
		...
		multi_role = 1,
		cas = 1,
		tactical_bomber = 3, -- Bomb the Chinese!
		naval_bomber = 1};
       else laArray = {
		interceptor = 6,
		multi_role = 3,
...
function P.Build_infantry_brigade(vIC, viManpowerTotal, voType, voProductionData, viUnitQuantity)
	-- Build old style formations till 1942
	if voProductionData.Year < 1942 and not(voProductionData.IsExile) then
		voType.Size = 3
		voType.SecondaryMain = "artillery_brigade"
		voType.Support = 0
	end
...
-- Create Custom marine for Japan
...
function P.Build_Industry(vIC, voProductionData)
	if voProductionData.Year < 1942 then
	  ic = Support.Build_Industry(ic, voProductionData, 5425, 8) -- Hiroshima
		ic = Support.Build_Industry(ic, voProductionData, 5543, 10) -- Nagasaki
		ic = Support.Build_Industry(ic, voProductionData, 5641, 8) -- Kagoshima
		ic = Support.Build_Industry(ic, voProductionData, 7238, 8) -- Sapporo
		ic = Support.Build_Industry(ic, voProductionData, 5345, 6) -- Kyoto
